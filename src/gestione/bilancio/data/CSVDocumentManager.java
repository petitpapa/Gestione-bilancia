 ...
 * @author petitpapa
 */
public class CSVDocumentManager implements DocumentManager {
    ...
    public void read(File selectedFile, TableView<Bilancio> table) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(selectedFile))) {
            ...
            while ((line = br.readLine()) != null) {
                String[] tokens = null;
                if (!line.isEmpty())
                    tokens = line.split(COMA_SEPARATOR);
            ...
            Logger.getLogger(CSVDocumentManager.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void save(File file, TableView<Bilancio> data) {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(file))) {
            data.getItems().stream().forEach(e -> {
                try {
                    bw.write(new StringJoiner(COMA_SEPARATOR).add(e.getDate().toString()).add(e.getDescription()).add(String.valueOf(e.getAmmontare())).add(e.getTipo().name()).toString());
